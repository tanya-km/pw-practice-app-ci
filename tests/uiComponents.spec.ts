...
dialog.accept()
 ...
 test('web tables', async({page}) => {
  await page.getByText('Tables & Data').click()
   ...
   //all the test data what we want to use
   const ages = ["20", "30", "40", "200"]
   //we created a loop to loop through each of the values and then using each of the age
   for(let age of ages){
    //to find filter of Age 
    await page.locator('input-filter').getByPlaceholder('Age').clear()
    //to type value of the age
   await page.locator('input-filter').getByPlaceholder('Age').fill(age)
   await page.waitForTimeout(500)
   //to find all rows as a result of the search 
const ageRows = page.locator('tbody tr')

//to loop through each of the rows 
